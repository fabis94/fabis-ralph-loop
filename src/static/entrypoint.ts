// Generated by fabis-ralph-loop — DO NOT EDIT MANUALLY
...
const shadowVolumesRaw = process.env.RALPH_SHADOW_VOLUMES || ''
const shadowVolumes = shadowVolumesRaw ? shadowVolumesRaw.split(',').map((s) => s.trim()) : []

for (const sv of shadowVolumes) {
  try {
    execSync(`sudo mkdir -p ${sv}`, { stdio: 'pipe' })
    execSync(`sudo chown -R node:node ${sv}`, { stdio: 'pipe' })
  } catch (error) {
...
if (existsSync('/workspace/.envrc')) {
  try {
    ...
    const allowDir = join(process.env.HOME || '/home/node', '.local/share/direnv/allow')
